...
describe("MarketplaceService", () => {
  describe("getPlugins", () => {
    it("should return PluginsMap", async () => {
      ...
      const result = await MarketplaceService.getPlugins(1, 2);
      expect(result).toStrictEqual(mockedResult);
      expect(hoisted.mockGet).toBeCalledWith(
        `${MARKETPLACE_URL}/plugins?skip=${1}&take=${2}`
      );
    });
    it("should throw and error and write to console.error", async () => {
      ...
      await expect(MarketplaceService.getPlugins(1, 2)).rejects.toThrow(
        "From test"
      );
  ...
  describe("getPolicies", () => {
    it("should return PluginPolicy", async () => {
      ...
      hoisted.mockGet.mockResolvedValueOnce(mockedResult);
      const result = await MarketplaceService.getPolicies("test");
      expect(result).toStrictEqual(mockedResult);
      expect(hoisted.mockGet).toBeCalledWith(
        `${MARKETPLACE_URL}/plugins/policies`,
        {
          headers: {
            plugin_type: "test",
            public_key: "null",
    ...
    it("should throw and error and write to console.error when mesage is not Unauthorized", async () => {
      hoisted.mockGet.mockRejectedValueOnce(new Error("From test"));
      const consoleSpy = vi.spyOn(console, "error");
      await expect(MarketplaceService.getPolicies("test")).rejects.toThrow(
        "From test"
      );
      expect(consoleSpy).toBeCalledWith(
  ...
  describe("getPolicyTransactionHistory", () => {
    it("should return PolicyTransactionHistory", async () => {
      ...
      hoisted.mockGet.mockResolvedValueOnce(mockedResult);
      const result =
        await MarketplaceService.getPolicyTransactionHistory("test");
      expect(result).toStrictEqual(mockedResult);
      expect(hoisted.mockGet).toBeCalledWith(
        `${MARKETPLACE_URL}/plugins/policies/test/history`,
        {
          headers: {
            public_key: "null",
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
    ...
    it("should throw and error and write to console.error", async () => {
      ...
      const consoleSpy = vi.spyOn(console, "error");
      await expect(
        MarketplaceService.getPolicyTransactionHistory("test")
      ).rejects.toThrow("From test");
      expect(consoleSpy).toBeCalledWith(
